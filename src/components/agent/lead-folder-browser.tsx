...
import { createLeadFolder, deleteLeadFolder, renameLeadFolder } from "@/app/dashboard/actions";
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        <div>
          <h2 className="font-semibold text-ink">Lead folders</h2>
          <p className="text-xs text-slate-500">Organize leads like Drive.</p>
        </div>
          ...
          return (
            <div key={folder.id} className={`group flex items-center rounded-lg ${active ? "bg-blue-50" : "hover:bg-slate-50"}`} style={{ paddingLeft: `${folder.depth * 14}px` }}>
              <Link href={folderHref(folder.id)} className={`flex min-w-0 flex-1 items-center gap-1.5 px-2 py-2 text-sm ${active ? "font-semibold text-brand-blue" : "text-slate-600"}`}>
                ...
                <span className="ml-auto text-xs text-slate-400">{folder.lead_count}</span>
              </Link>
